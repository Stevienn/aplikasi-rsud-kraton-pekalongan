...
import { useGetUserById, useGetUsers } from "@/hooks/api/useUser";
...
const RekapMedisPasien = ({ params }: Props) => {
  ...
  const { data: rekapMedis } = useGetUserById(slug);
  ...
  const rows = useMemo(() => {
    ...
    return rekapMedis.rekap_medis.map((data, index) => ({
      id: index + 1,
      ...
      <div className="bg-light-primary px-[55px] py-[30px] h-[88dvh] font-inria-sans">
        <div className="flex gap-[10px] mb-[20px]">
          <h1 className="text-[30px] font-semibold font-inter-sans">
            Rekap Medis Pasien {rekapMedis?.nama} , No BPJS
          </h1>
